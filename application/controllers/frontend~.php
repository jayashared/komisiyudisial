...
class Frontend extends CI_Controller {
	...
	public function index() 
	{
		...
		$data["slider"] = $this->slider_m->get_latest(6);
		...
		{	
			...
			else
			{
				if( $sess == "id" )
				{
					$m = '
						<div class="col-md-12 bg-primary">
							<b>Gagal</b> Silahkan kirim email ke humas@komisiyudisial.go.id
						</div>
					';
				}
				else
				{
					$m = '
						<div class="col-md-12 bg-warning">
							<b>Failed</b> Please send email to humas@komisiyudisial.go.id
						</div>
					';
				}
			}
